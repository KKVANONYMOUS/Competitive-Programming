...
bool checkRightTriangle(vector <pii> p){
    int a=pow(p[0].F-p[1].F,2)+pow(p[0].S-p[1].S,2);
    int b=pow(p[1].F-p[2].F,2)+pow(p[1].S-p[2].S,2);
    int c=pow(p[0].F-p[2].F,2)+pow(p[0].S-p[2].S,2);
...
void solve(){
    ...
    vii copy_p=p;
    if(checkRightTriangle(p)) cout<<"RIGHT";
    else{
        vi diff_x={-1,0,1,0};
        vi diff_y={0,-1,0,1};
        ...
        fl(i,0,3){
            fl(j,0,4){
                p[i].F=p[i].F+diff_x[j];
                p[i].S=p[i].S+diff_y[j];
