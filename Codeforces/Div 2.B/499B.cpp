...
void solve(){
   int n,m;
   cin>>n>>m;
   vector <pair<string,string>> key(m);
   fl(i,0,m) cin>>key[i].F>>key[i].S;
   vector <string> lec(n);
   ...
   fl(i,0,n){
       ...
       int index=0;
       fl(j,0,m){
           if (key[j].F==lec[i] or key[j].S==lec[i])  index=j;
       }
       string ans=sz(key[index].F)==sz(key[index].S)?key[index].F:sz(key[index].F)<sz(key[index].S)?key[index].F:key[index].S;
       notes[i]=ans;
   }
   fl(i,0,n) cout<<notes[i]<<" ";
